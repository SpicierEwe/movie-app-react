...
export default function FavouritesComponent() {
  ...
  const filterFavourites = () => {
    let filteredMovies = movies.filter((movie) => {
      return favouritesIdList.includes(movie.id);
    });
    return filteredMovies;
  };

  //   TO INCREASE PERFORMANCE, WE CAN USE useEffect TO FILTER FAVOURITES
  useEffect(() => {
    setFilteredFavourites(filterFavourites());
  }, [favouritesIdList]);
    ...
    <div>
      {!isLoading && (
        <div className="ml-auto  mr-auto mt-12 mb-12 pt-16 grid grid-cols-4 gap-12 max-w-screen-xl">
          {filterFavourites.map((movie, index) => (
            <div
              key={index}
