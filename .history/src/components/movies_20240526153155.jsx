      ...
      {!isLoading && (
        <div className="ml-auto  mr-auto mt-12 mb-12 grid grid-cols-4 gap-12 max-w-screen-xl">
          {movies.map((movie, index) => (
            <div key={index}>
              <div className="rounded-lg w-64 h-96 ">
                <img src={link} alt="movie" className="object-cover relative" />
                <div className="mt-4 absolute z-10">
                  <h1 className="text-xl font-bold">{movie.rating}</h1>
                  <p className="text-gray-500">{movie.movie}</p>
                </div>
              </div>
            </div>
          ))}
